...
把最常看的四个数字常驻在手机通知栏，下拉就能看到，不用打开 App：

| 位置 | 内容 |
| --- | --- |
| 第一行（收起态可见） | 预算状态 + 已用百分比，例如「🔴 预算告急 · 已用 132.1%」 |
| 第二行（收起态可见） | 今日支出 / 今日收入 / 本月结余 |
| 进度条 | 预算使用进度，颜色随预警等级变化 |
| 展开后追加 | 今日支出与收入明细、本月预算 / 已用 / 剩余或超支、本月结余 |

#### 预警配色

按后端返回的预算使用率自动切换，颜色同时作用在**背景色**与**进度条**上：
...
| 70% ~ 90% | 🟡 橙 | 注意控制 |
| ≥ 90% 或已超支 | 🔴 红 | 预算告急 |
| 未设置预算 | ⚪ 灰 | 未设置预算（只显示支出与结余） |

#### 刷新机制

- 后台每 **30 分钟**自动刷新一次（`LedgerScheduler.INTERVAL_MS` 改这一处即可调整）；
- 打开 App、从后台切回前台、记录 / 修改 / 删除一笔账单后**立即刷新**；
- 手机重启后自动恢复（监听 `BOOT_COMPLETED`）；
- 退出登录时自动收起，重新登录后按上次的开关状态自动恢复。

#### 开启方式

「我的 → 通知栏看板」打开开关。Android 13 及以上会弹出系统通知授权框，
授权后通知立即出现；拒绝则开关回到关闭状态并给出提示。

#### 实现位置

功能全部在 `frontend/` 内：原生侧为 `android/app/src/main/kotlin/.../Ledger*.kt`
（取数、渲染、调度、接收器、开播恢复、MethodChannel 桥）与两套 `res/layout/ledger_notification*.xml`
布局；Flutter 侧为 `services/ledger_notification_service.dart`，并在「我的」页面接入开关。

四个数字全部来自**已有接口**（`/statistics/daily-summary`、`/statistics/monthly`、`/budgets`），
**后端零改动、数据库零改动、零新增依赖**。
...
| 限制 | 说明 |
| --- | --- |
| 小米 / HyperOS 会收纳 | MIUI 把该通知归入「更多通知」，下拉后需再点一下才看到。渠道优先级在 LOW / DEFAULT / HIGH 三档都实测过，MIUI 一律收纳，属 ROM 行为，App 无法改变；其它品牌无此问题 |
| 需要允许自启动 | HyperOS 等系统清理后台后会中断定时刷新，建议在系统设置中允许「福瑞记账」自启动 |
| 深度休眠会延后 | Android 的 Doze 机制可能把刷新推迟到维护窗口，属系统行为 |
| 强制停止后失效 | 用户手动「强行停止」App 时，Android 会清除其通知与闹钟，需重新打开 App |
| 仅 Android | 通知栏看板依赖 Android 的 NotificationManager；Web / Windows 不显示，会静默跳过 |
...
| Stage 5-A | 通知栏常驻看板（Android 原生：今日开支 / 本月预算 / 结余 + 预警配色 + AlarmManager 后台刷新；后端与数据库零改动、零新增依赖） | 已完成 |
...
### 13.3 通知栏看板真机验证（2026-09-20）

| 项 | 内容 |
| --- | --- |
| 设备 | Redmi 23113RKC6C（Android 16 / HyperOS V816） |
| 安装包 | `flutter build apk --release --dart-define=API_BASE_URL=http://129.204.61.68/api` → 59.3 MB |
| 授权流程 | 「我的 → 通知栏看板」打开 → 系统通知授权框 → 允许后通知立即出现；拒绝则开关回退为关闭并给出提示 |
| 通知记录 | `flags=ONGOING_EVENT\|ONLY_ALERT_ONCE`、`color=0xffd64545`（告急红）、渠道 `ledger_dashboard_v2` |
| 收起态渲染 | 粉色告急底 + 「🔴 预算告急 · 已用 132.1%」+「今日支 ¥0 · 收 ¥0 · 结余 ¥623」+ 红色进度条 |
| 展开态渲染 | 追加今日支出与收入明细、本月预算 / 已用 / 超支、本月结余，共五行 |
| 数据一致性 | 通知里的本月结余 ¥623、预算使用率 132.1% 与接口返回逐字一致 |
| 开关联动 | 退出登录后通知自动收起；重新登录后按上次开关状态自动恢复 |

过程中踩到并解决的三个问题：

| # | 现象 | 根因 | 处理 |
| --- | --- | --- | --- |
| 1 | release 包安装后**启动即崩** | 定时刷新最初用 WorkManager，release 构建的 R8 削掉了它依赖的 Room 生成类（`NoSuchMethodException: WorkDatabase_Impl.<init>`） | 改用系统自带的 `AlarmManager` 调度，**最终零新增依赖** |
| 2 | 收起态装不下完整内容 | 系统给收起态的高度有限，四行文字加进度条会被截断 | 拆成两套布局：收起态两行 + 进度条，展开态完整五行 |
| 3 | 通知被收进「更多通知」 | 小米 HyperOS 自行把它判定为不重要通知 | 渠道优先级 LOW / DEFAULT / HIGH 三档均实测，MIUI 一律收纳，属 ROM 行为；已按 DEFAULT 定稿并记入「已知限制」 |
